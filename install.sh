...
install_core_packages() {
    ...
    echo "$packages" | sudo xargs pacman -S  # BUG: pacman od razu konczy i nie daje wpisac y/n
}
...
install_aur_helper() {
    ...
    cd aura-bin || exit
    makepkg
    aur_package=$(ls -l | grep -E 'aura-bin-[0-9].*pkg.*' | awk '{print $NF}')
...
install_display_manager() {
    ...
    if [ "$display_manager" = "gdm" ]; then
        gum confirm "Install gdm-settings?" && sudo aura -A gdm-settings
    fi
...
# Core setup
sudo pacman -S jq gum  # Packages needed for install_core_packages()
install_core_packages
